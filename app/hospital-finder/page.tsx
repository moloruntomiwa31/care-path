      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            if (isMounted) {
              setHospitals([]);
              setToast({ message: 'Failed to fetch hospitals. Please check your connection and refresh the page.', type: 'error' });
            }
          } finally {
        ...
        (error) => {
          ...
          if (isMounted) {
            setLoading(false);
            setToast({ message: 'Unable to get your location. Please enable location services.', type: 'error' });
          }
        }
      );
    };
			...
			<div className="flex-1 flex flex-col">
				<header className="bg-white shadow-sm px-4 py-3 flex items-center justify-between z-10 lg:pl-4 pl-16">
					<div className="flex flex-col justify-center h-10">
						...
						<p className="text-xs text-gray-500">Find nearby healthcare facilities</p>
					</div>
					<button
